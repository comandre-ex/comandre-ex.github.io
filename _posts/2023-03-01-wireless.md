...
Sería una pena que mientras jugamos nos identificaran la ```DIRECCIÓN MAC```,Para ello cambiaremos nuestra ```DIRECCIÓN MAC``` haciéndonos pasar por otro dispositivo.
...
Cuando conectamos nuestro móvil a un ```AP``` como el de casa de tu abuela y luego nos vamos pero volvemos en una semana, el dispositivo se conecta automáticamente, ¿no? El dispositivo se reasociará y enviará la contraseña cifrada ```(HANDSHAKE)``` al ```AP``` de la misma manera que la primera vez que conectamos nuestro dispositivo al ```AP```.
 lo que significa que podemos capturar el ```(Hanshake)``` cuando el móvil se reasocia. Para luego descifrar esa contraseña pero no vamos a esperar cada vez que un cliente se conecte o se vuelva a asociar. Entonces desconectaremos los clientes que están en la RED. Enviando paquetes de desautenticación al AP usando la `MAC  DE BROADCAST` como objetivo para poder desconectar a todos los clientes.
